...
local vmajor, vminor = "CandyBar-2.0", 90000 + tonumber(("$Revision: 158 $"):match("(%d+)"))
...
function CandyBar:OnClick(self, arg1)
	if not self.owner then
		return
	end
	local handler = CandyBar.handlers[self.owner]
	if not handler then
		return
	end
	...
	local button = arg1
	handler.onclick(self.owner, button, getArgs(handler, "onclick"))
	return true
end
...
-- on update handler
local lastSlow = 0
function CandyBar_OnUpdate(self, arg1)
	local doslow
	...
	local t
	for i,v in pairs(self.owner.handlers) do
...
local function activate(self, oldLib, oldDeactivate)
	...
	self.frame:SetScript("OnUpdate", CandyBar_OnUpdate)
